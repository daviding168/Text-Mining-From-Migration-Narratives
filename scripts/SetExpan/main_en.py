...
from scripts.SetExpan.set_expan_standalone_en import setExpan
...
level2source_weights = {
    0: {"sg": 1.0, "tp": 1.0, "eb": 1.0},
    1: {"sg": 1.0, "tp": 1.0, "eb": 1.0}
}
...
level2threshold = {"en": {"Accommodation": 0.74, "Means of transport": 0.72, "Environment": 0.7, "Family Members": 0.75}}
...
def runMultiSetExpan(iteration_number=10, lang="en", threshold_boolean=False, debug=False):
    ...
    while update:
        ...
        if threshold_boolean:
            print("Truncating MultiSetExpan based on predefined threshold...")
            nodesToTruncate = []
            for node in rootNode.children:
                for child in node.children:
            ...
            for node in nodesToTruncate:
                node.parent.cutFromChild(node)
                node.delete()
        print("=== MultiSetExpan at iteration %s ===" % iters)
...
if __name__ == "__main__":
    ...
    print(subtract_annotated_taxonomy)
    print()
    ...
    print("After ", iter_num, " iterations")
