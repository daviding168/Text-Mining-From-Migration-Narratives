...
# Skip-grams that can cover [FLAGS_SG_POPULARITY_LOWER, FLAGS_SG_POPULARITY_UPPER] numbers of entities will be retained
FLAGS_SG_POPULARITY_LOWER = 1
FLAGS_SG_POPULARITY_UPPER = 100
...
def getSampledCoreSkipgrams(coreSkipgrams):
    """
    Generates number of random samples with a specified size, without replacement, from a list of strings.
...
def getCombinedWeightByFeatureMap(seedEids, featuresByEidMap, weightByEidAndFeatureMap):
    """
    function to combine the weight of context or type features given a list of seedEid
    """
    combinedWeightByFeatureMap = {}
    for seed in seedEids:
        # set of context or type feature given an eid
        featuresOfSeed = featuresByEidMap[seed]
        # loop through each context or type feature
...
def setExpan(seedEidsWithConfidence, negativeSeedEids, eid2patterns, pattern2eids, eidAndPattern2strength,
    ...
    # test single setExpan
    # seedEids = [list(eid2ename.values()).index(ele[0]) for ele in seedEidsWithConfidence]
    # dict of eid(key) and confidence_score(value)
    # eid2confidence = {ele[0]: ele[1] for ele in seedEidsWithConfidence}
    # eid2confidence = {list(eid2ename.values()).index(ele[0]): ele[1] for ele in seedEidsWithConfidence}
    ...
    while iters < MAX_ITER_SET:
        ...
        # eid2mrr = {key: value for key, value in eid2mrr.items() if value >= THRES_MRR}
        ...
        eid_incremental = []
        # loop through sorted (eid, score) in descending order based on mrr score and select MAXIMUM_EXPAND entities to expand
    ...
    for eid in seedEids:
        if eid not in cached_seedEids:
            ...
            # view_expanded.append((eid2ename[eid], eid2confidence[eid]))
    # print(view_expanded)
    return expanded
